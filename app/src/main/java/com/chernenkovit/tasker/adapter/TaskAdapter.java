...
/** Custom adapter with common data for current and done tasks adapters. */
        ...
        if (location >= 0 && location <= getItemCount() - 1) {
            ...
            if (location - 1 >= 0 && location <= getItemCount() - 1) {
                if (!getItem(location).isTask() && !getItem(location - 1).isTask()) {
                    ModelSeparator modelSeparator = (ModelSeparator) getItem(location - 1);
                    checkSeparator(modelSeparator.getType());
